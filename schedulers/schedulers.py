...
class LinearScheduler(BaseScheduler):
    ...
    def step_epoch(self, verbose=True):
        ...
        if verbose:
            logger.info("Epoch {:3d} eps start {:7.5f} end {:7.5f}".format(self.epoch, self.epoch_start_eps,
                                                                           self.epoch_end_eps))

    def step_batch(self):
        if self.schedule_length == 0:
            super(LinearScheduler, self).step_batch()
            return
        if self.is_training:
            self.batch += 1
            ...
            if self.batch > self.epoch_length:
                logger.warning(
                    'Warning: we expect {} batches in this epoch but this is batch {}'.format(self.epoch_length,
                                                                                              self.batch))
...
class SmoothedScheduler(BaseScheduler):
    ...
    def __init__(self, max_eps, opt_str):
        ...
        logger.debug(f"SmoothedScheduler: start {self.schedule_start}, length {self.schedule_length}, "
                     f"mid {self.mid_point}, beta {self.beta}, max_eps {self.max_eps}")
    ...
    def step_batch(self, verbose=False):
        if self.schedule_length == 0:
            super(SmoothedScheduler, self).step_batch()
            return
        if self.is_training:
            self.batch += 1
            init_value = 0.0
            ...
            linear_value = min(mid_value + (final_value - mid_value) * (step - mid_step) / (final_step - mid_step),
                               final_value)
            self.eps = is_ramp * ((1.0 - is_linear) * exp_value + is_linear * linear_value) + (
    ...
    def get_eps(self):
        if self.schedule_length == 0:
            ...
            return super(SmoothedScheduler, self).get_eps()
