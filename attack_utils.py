...
class AttackUtils(object):
        ...
        def __init__(self, lower_limit, upper_limit, std,
                ...
                self.std = std
                self.device = device
                self.lower_limit = torch.tensor(lower_limit).to(device)
                self.upper_limit = torch.tensor(upper_limit).to(device)
        ...
        def evaluate_noise(self, test_loader, model, sigma=0.1):
                ...
                for i, batch in enumerate(test_loader):
                        X, y = batch[0].to(self.device), batch[1].to(self.device)
                        # Noise to add
                        noise = torch.normal(mean=0, std=sigma, size=X.shape).to(self.device)
